...
time_data = np.linspace(0, 4, total_points)  # Time array from 0 to 4 seconds
...
with TdmsWriter(file_path) as tdms_writer:
    ...
    time_channel = ChannelObject("Waveforms", "A1_Time", time_data)
    signal_channel = ChannelObject("Waveforms", "A1", signal)
